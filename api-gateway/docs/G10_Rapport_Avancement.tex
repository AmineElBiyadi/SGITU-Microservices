...
G10 verifie la signature, l'expiration, le \texttt{sub} et au moins un role dans \texttt{roles} ou \texttt{role}. Il ne consulte pas une table locale des utilisateurs.
...
G3 & \texttt{/auth/**}, \texttt{/api/users/**}, \texttt{/api/profiles/**} & public/auth, JWT, admin pour roles/desactivation \\
\rowcolor{lightgray}
...
\rowcolor{lightgray}
G8 Analytics & \texttt{/api/v1/ingestion/**}, \texttt{/api/v1/analytics/**} & JWT, admin/agent analytics \\
\rowcolor{lightgray}
G8 ML & \texttt{/predict/peak-hours}, \texttt{/predict/incidents} & JWT, admin/agent \\
