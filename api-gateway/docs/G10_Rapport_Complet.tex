...
\diagram{diagrams/g10_10_sequence_g5_g8_analytics.png}{0.95}{Flux G5 vers G8 via G10 pour l'ingestion analytique}
...
G8 Analytics & \texttt{/api/v1/ingestion/**}, \texttt{/api/v1/analytics/**} & \texttt{g8-analytics:8088} & JWT, admin/operator/staff analytics \\
G8 ML & \texttt{/predict/peak-hours}, \texttt{/predict/incidents} & \texttt{ml-service:5000} & JWT, admin/operator/staff \\
...
POST & \texttt{/api/users} & G3 & Public, creation de compte \\
POST & \texttt{/auth/refresh} & G3 & Public \\
POST & \texttt{/auth/logout} & G3 & JWT, revocation token \\
\bottomrule
\end{longtable}
...
\midrule
GET & \texttt{/api/users} & Liste des utilisateurs & ROLE\_ADMIN \\
...
\begin{itemize}
  ...
  \item \texttt{g8-analytics:8088} ;
  \item \texttt{ml-service:5000} ;
...
\midrule
\texttt{/auth/login}, \texttt{/auth/refresh} & Public, route vers G3. \\
\texttt{/auth/logout} & JWT requis, route vers G3 pour revocation. \\
\texttt{POST /api/users} & Public, creation de compte routee vers G3. \\
\texttt{GET /api/users} & \texttt{ROLE\_ADMIN}. \\
\texttt{/api/users/*/roles} & \texttt{ROLE\_ADMIN}. \\
\texttt{/api/users/*/activate} & \texttt{ROLE\_ADMIN}. \\
\texttt{/api/users/*/deactivate} & \texttt{ROLE\_ADMIN}. \\
\texttt{DELETE /api/users/*} & \texttt{ROLE\_ADMIN}. \\
...
\texttt{/api/v1/analytics/**}, \texttt{/predict/**} & \texttt{ROLE\_ADMIN}, \texttt{ROLE\_OPERATOR} ou \texttt{ROLE\_STAFF}. \\
